...
Confirmed directly against the live DB: the one existing user (`smitsatasiya7@gmail.com`, created via the `/setup` wizard's password sign-up) had `email_verified = false` and only a single `credential` (password) row in `account` — exactly the state that trips Better Auth's `requireLocalEmailVerified` gate on a later Google attempt.
