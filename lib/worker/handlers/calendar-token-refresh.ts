...
async function processCalendarTokenRefresh(
  job: Job<CalendarTokenRefreshPayload>
) {
  ...
  try {
    ...
    const encryptedRefresh = newRefresh
      ? await encrypt(newRefresh)
      : cal.refreshToken;
    ...
    console.log(
      `[calendar-token-refresh] refreshed token for ${connectedCalendarId}`
    );
  } catch (err) {
