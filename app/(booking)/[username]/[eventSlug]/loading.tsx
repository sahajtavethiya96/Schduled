      ...
      <div className="mx-auto w-full max-w-[900px] overflow-hidden bg-base-100 border border-base-300 lg:flex lg:h-full lg:max-h-[680px] lg:flex-col">
        ...
        <div className="flex flex-1 flex-col overflow-hidden lg:flex-row">
            ...
            <div className="grid grid-cols-7 gap-y-1">
              {Array.from({ length: 35 }).map((_, i) => (
                <div key={i} className="flex items-center justify-center p-0.5">
                  <div className="h-9 w-9 rounded-full bg-base-200 animate-pulse" style={{ animationDelay: `${i * 20}ms` }} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-1 flex-col justify-center gap-4 p-6">
            <div className="h-3 w-20 bg-base-200 animate-pulse" />
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-14 w-full bg-base-200 animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
            ))}
          </div>
        </div>
      </div>
